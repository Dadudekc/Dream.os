...
class FeedbackEngine:
    ...
    def __new__(cls, *args, **kwargs):
        """Ensure only one instance of FeedbackEngine exists."""
        if cls._instance is None:
            logger.info("🧠 Creating new FeedbackEngine singleton instance")
            cls._instance = super(FeedbackEngine, cls).__new__(cls)
        return cls._instance

    def __init__(self, memory_file: str = "memory/persistent_memory.json", feedback_log_file: str = "memory/feedback_log.json"):
        """Initialize the FeedbackEngine singleton if not already initialized."""
        # Skip initialization if already done
        if FeedbackEngine._initialized:
            logger.info("⚠️ FeedbackEngine already initialized, skipping redundant init")
            return
        ...
        self._load_memory()
        FeedbackEngine._initialized = True  # Mark as initialized
